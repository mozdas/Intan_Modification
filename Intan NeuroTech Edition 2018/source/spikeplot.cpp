...
SpikePlot::SpikePlot(SignalProcessor *inSignalProcessor, SignalChannel *initialChannel,
                     SpikeScopeDialog *inSpikeScopeDialog,PsthDialog *inPsthDialog,bool inDialogType, QWidget *parent) :
    QWidget(parent)
{
...
void SpikePlot::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        if (frame.contains(event->pos())) {
            ...
            if(!dialogType && newThreshold>0) newThreshold=0;
            setVoltageThreshold(newThreshold);
            if(!dialogType){
                psthDialog->setVoltageThresholdDisplay(newThreshold);
                if(!(psthDialog->getThresholdType())){

                    psthDialog->setVoltageThreshold(newThreshold);
                }
            }


            if(dialogType)spikeScopeDialog->setVoltageThresholdDisplay(newThreshold);
...
void SpikePlot::setVoltageTriggerMode(bool voltageMode)
{
    if(dialogType){
        ...
        voltageTriggerMode=1;
        if (selectedChannel->signalType == AmplifierSignal) {
            selectedChannel->voltageTriggerMode =1;
        }
    }
