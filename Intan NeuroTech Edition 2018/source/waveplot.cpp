...
SignalSources* WavePlot::getSignalSources(){
    return signalSources;
}

void WavePlot::setNewSelectedChannel(SignalChannel *newSelectedChannel){

    int index = 0;
    bool selectedChannelFound = false;

    //check each channel in the port to detect a change
    while(!selectedChannelFound && (index < signalSources->signalPort[selectedPort].numAmplifierChannels())){
        selectedChannelFound = ( newSelectedChannel == signalSources->signalPort[selectedPort].channelByIndex(index) );
        if(!selectedChannelFound){
            index++;
        }
    }

    if(selectedChannelFound){
        //highlightFrame(index , true);
        changeSelectedFrame(index,false);
    }
}
void WavePlot::arrengeDisplayOrder( QVector<int> channelNumbersForMapping){
    if(channelNumbersForMapping.size() > signalSources->signalPort[selectedPort].numAmplifierChannels()){
        return;
    }

    for (int i = 0 ; i < channelNumbersForMapping.size() ; ++i){
        signalSources->signalPort[selectedPort].channelByNativeOrder(channelNumbersForMapping[i])->userOrder = i;
    }
