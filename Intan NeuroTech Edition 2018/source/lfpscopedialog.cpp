    ...
    QDialog(parent)
{
    ...
    resetLfpButton = new QPushButton(tr("Set Channel to the Selected One"));
    ...
    connect(resetLfpButton,SIGNAL(clicked()),
            this,SLOT(resetLfp()));
    ...
    leftLayout->addWidget(resetLfpButton);
...
void LfpScopeDialog::resetLfp(){
    lfpPlot->setNewChannel(futureChannel);
    digInPlot->setNewChannel(futureChannel);

    if(lfpAnalysisDialog){

        lfpAnalysisDialog->setNewChannel(futureChannel);

    }

    currentChannel = futureChannel;
    if (currentChannel->voltageTriggerMode) {
        triggerTypeComboBox->setCurrentIndex(0);
    } else {
        triggerTypeComboBox->setCurrentIndex(1);
    }

    thresholdSpinBox->setValue(currentChannel->voltageThreshold);
    digitalInputComboBox->setCurrentIndex(currentChannel->digitalTriggerChannel);
    if (currentChannel->digitalEdgePolarity) {
        edgePolarityComboBox->setCurrentIndex(0);
    } else {
