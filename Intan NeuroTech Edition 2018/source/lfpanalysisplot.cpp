    ...
    if(analysisType == MIN_ANALYSIS ){
        ...
        if(numLfpWaveforms==0){
            ...
            for (int i = 0; i < numLfpWaveforms; ++i) {
                ...
                //search minimum as adjusted in the comboBox
                for(int j=windowStartTimeIndex ; j < windowEndTimeIndex; ++j){
                    if(temp_min>lfpPlot->lfpWaveform.at((i+offsetVal+30)% 30) .at(j)){
                        temp_min=lfpPlot->lfpWaveform.at((i+offsetVal+30)% 30) .at(j);
                ...
                double temp_base=0;
                for(int j=0;j<lfpPlot->preTriggerTSteps;j++){

                    temp_base+=lfpPlot->lfpWaveform.at((i+offsetVal+30)% 30) .at(j);

                }
                temp_base=temp_base/lfpPlot->preTriggerTSteps;
                singleShotLfpData[i] = temp_base - temp_min;
                temp_av +=singleShotLfpData[i];
            }
            temp_av = temp_av/numLfpWaveforms;
            ...
            for(int j=0;j<lfpPlot->preTriggerTSteps;j++){
                mean_base+=lfpPlot->meanLfpWaveform.at(j);
            }

            mean_base=mean_base/lfpPlot->preTriggerTSteps;
            meanSingleShotLfpData = mean_base - minSingleShotLfpData;
            //Calculate the Standard Deviation for the ErrorBar
            ...
            if (numLfpWaveforms >1){
                ...
                baseValues.resize(numLfpWaveforms);
                baseValues.fill(0.0);
                minValues.resize(numLfpWaveforms);
                minValues.fill(0.0);

                for(int k=0;k<numLfpWaveforms;k++){
                    for(int j=0;j<lfpPlot->preTriggerTSteps;j++){
                        baseValues[k]+=lfpPlot->lfpWaveform.at((k+offsetVal+30)%30).at(j);
                    }
                    baseValues[k]=baseValues[k]/lfpPlot->preTriggerTSteps;
                    minValues[k]=lfpPlot->lfpWaveform.at((k+offsetVal+30) % 30).at(minPointIndex);
                }

                //From the minimum of mean LFP response;
                for (int i = 0; i < numLfpWaveforms; ++i) {
                    int diff_value = baseValues[i]-minValues[i];
