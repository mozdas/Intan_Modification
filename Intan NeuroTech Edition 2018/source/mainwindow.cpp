...
MainWindow::MainWindow()
{
    ...
    probeDisplayWidget = new ProbeDisplayWidget(wavePlot, this);

    connect(wavePlot, SIGNAL(selectedChannelChanged(SignalChannel*)),
            this, SLOT(newSelectedChannel(SignalChannel*)));

    /*connect(probeDisplayWidget, SIGNAL(selectedChannelChanged(SignalChannel*)),
            this, SLOT(newSelectedChannel(SignalChannel*)));*/
    ...
    setSaveFormat(SaveFormatFilePerChannel);
...
void MainWindow::createLayout()
{
    ...
    lfpScopeButton = new QPushButton(tr("Open LFP Scope"));
    spikeScopeButton = new QPushButton(tr("Open Spike Scope"));
    magnifyButton=new QPushButton(tr("Magnify"));
    psthButton=new QPushButton(tr("PSTH"));
    ...
    numWaveformsLayout->addWidget(psthButton);

    numWaveformsLayout->addWidget(lfpScopeButton);

    connect(spikeScopeButton, SIGNAL(clicked()),
            this, SLOT(spikeScope()));
    connect(lfpScopeButton, SIGNAL(clicked()),
            this, SLOT(lfpScope()));
    connect(psthButton,SIGNAL(clicked()),this,SLOT(psth()));
    connect(magnifyButton,SIGNAL(clicked()),
            this,SLOT(magnify()));
    ...
    scaleLayout->addWidget(magnifyButton);
    ...
    midLayout->addWidget(groupBox1);
    midLayout->addWidget(tabWidget1);
    midLayout->addStretch(1);

    QHBoxLayout *mainLayout = new QHBoxLayout;
    mainLayout->addWidget(probeDisplayWidget);
...
void MainWindow::changeSampleRate(int sampleRateIndex)
{
    ...
    if(magnifyDialog){
        magnifyDialog->setSampleRate(boardSampleRate);
    }
    if(psthDialog){
        psthDialog->setSampleRate(boardSampleRate);
    }

    if (lfpScopeDialog) {
        lfpScopeDialog->setSampleRate(boardSampleRate);
    }
...
void MainWindow::runInterfaceBoard()
{
    ...
    while (running) {
        ...
        if (newDataReady) {
            ...
            if(magnifyDialog){
                magnifyDialog->updateWaveform(numUsbBlocksToRead);
            }
            if (psthDialog) {
                psthDialog->updatePsthArray(numUsbBlocksToRead);
                psthDialog->updateWaveform(numUsbBlocksToRead);

            }
            if (lfpScopeDialog) {
                lfpScopeDialog->updateWaveform(numUsbBlocksToRead);
            }
...
void MainWindow::magnify(){
    if(!magnifyDialog){
        magnifyDialog=new MagnifyDialog(signalProcessor,signalSources,
                                        wavePlot->selectedChannel(),this);
    }
        magnifyDialog->show();
        magnifyDialog->raise();
        magnifyDialog->activateWindow();
       // magnifyDialog->setYScale(yScaleComboBox->currentIndex());
        magnifyDialog->setSampleRate(boardSampleRate);
        wavePlot->setFocus();
...
void MainWindow::psth()
{
    if (!psthDialog) {
        psthDialog = new PsthDialog(signalProcessor, signalSources,
                                                wavePlot->selectedChannel(), this);
        // add any 'connect' statements here
    }
    ...
    psthDialog->show();
    psthDialog->raise();
    psthDialog->activateWindow();
   // psthDialog->setYScale(yScaleComboBox->currentIndex());
    psthDialog->setSampleRate(boardSampleRate);
    wavePlot->setFocus();
}
void MainWindow::lfpScope()
{
    if (!lfpScopeDialog) {
        lfpScopeDialog = new LfpScopeDialog(signalProcessor, signalSources,
                                                wavePlot->selectedChannel(), this);
        // add any 'connect' statements here
    }

    lfpScopeDialog->show();
    lfpScopeDialog->raise();
    lfpScopeDialog->activateWindow();
    lfpScopeDialog->setYScale(yScaleComboBox->currentIndex());
    lfpScopeDialog->setSampleRate(boardSampleRate);
    wavePlot->setFocus();
}
...
void MainWindow::newSelectedChannel(SignalChannel* newChannel)
{
    ...
    if(magnifyDialog){
        magnifyDialog->setNewChannel(newChannel);
    }
    if (psthDialog) {
        psthDialog->setNewChannel(newChannel);
    }
    if (lfpScopeDialog) {
        lfpScopeDialog->setNewChannel(newChannel);

    }

    probeDisplayWidget->setNewChannel(newChannel);
